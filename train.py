...
solar = pd.merge(solar, sunspots[['period', 'days', 'smoothed_ssn', 'month']], 'left', ['period', 'days'])
...
# and wanted to avoid combinint different training periods into a single point.
# Later I changed to doing the rolling averages independently for each period, so I think this part is no longer needed.)
solar.loc[solar['period'] != 'train_a', 'timedelta'] \
    += solar.loc[solar['period'] == 'train_a', 'timedelta'].max() + pd.to_timedelta('100 days')
solar.loc[solar['period'] == 'train_c', 'timedelta'] \
    += solar.loc[solar['period'] != 'train_c', 'timedelta'].max() + pd.to_timedelta('100 days')
...
for p in ['train_a', 'train_b', 'train_c']:
    ...
    solar.loc[curr_period, 'train_exclude'] = solar.loc[curr_period, 'bad_data'].rolling(60 * 24 * 7, center=False).max()
...
for p in ['train_a', 'train_b', 'train_c']:
    ...
    solar.loc[curr_period, 'smoothed_ssn'] \
        = solar.loc[curr_period, 'smoothed_ssn'].fillna(method='ffill', axis=0).fillna(method='bfill', axis=0)
    roll = solar[train_short].rolling(window=20, min_periods=5).mean().interpolate('linear', axis=0)
    solar.loc[curr_period, train_short] = solar.loc[curr_period, train_short].fillna(roll)
    solar.loc[curr_period, train_short] \
        = solar.loc[curr_period, train_short].fillna(method='ffill', axis=0).fillna(method='bfill', axis=0)
...
for p in ['train_a', 'train_b', 'train_c']:
    ...
    new_df.loc[curr_period] \
        = solar.loc[curr_period, train_short].rolling(window=10, min_periods=1, center=False).agg(['mean', 'std']).values
...
trim1 = tf.keras.layers.Cropping1D((5, 0))(conv1)  # crop from left so resulting shape is divisible by 6
...
for p in ['train_a', 'train_b', 'train_c']:
    ...
    all_p_holdout = solar.loc[(solar['period'] == p) & valid_bool & holdout_bool].index.values[24 * 7:]
...
holdout_gen = DataGen(solar[train_cols].values, solar['dst_interp'].fillna(method='ffill').values.flatten(),
                      bs, holdout_ind, 6 * 24 * 7, False)
...
for n in range(num_iters):
    for cv_slice in range(num_slices):
        ...
        leave_out_months = train_months[cv_slice * (len(train_months) // num_slices): (cv_slice + 1) * (len(train_months) // num_slices)]
        leave_out_months_ind = solar.loc[valid_bool & solar['month'].isin(leave_out_months) & (~holdout_bool)].index.values
        curr_months_ind = solar.loc[valid_bool & (~solar['month'].isin(leave_out_months)) & (~holdout_bool)].index.values
        ...
        train_gen = DataGen(solar[train_cols].values, solar['dst_interp'].fillna(method='ffill').values.flatten(),
                            bs, train_ind, 6 * 24 * 7)
        test_gen = DataGen(solar[train_cols].values, solar['dst_interp'].fillna(method='ffill').values.flatten(),
                           bs, test_ind, 6 * 24 * 7)
        ...
        if test_mode:
            ...
            acc = np.sqrt(mean_squared_error(holdout_actuals, holdout_preds / (num_slices * n + (cv_slice + 1))))
            holdout_accuracy_arr.append(acc)
            print("Holdout accuracy for combined model: {}".format(acc))
        else:
            data_gen = DataGen(solar[train_cols].values, solar['dst_shift'].fillna(method='ffill').values.flatten(),
                               bs, train_ind, 6 * 24 * 7)
            tf.keras.backend.clear_session()
            model = tf.keras.Model(inputs, output)
            model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=tf.keras.optimizers.Adam(learning_rate=lr),
